...
- **AI-Assisted Plan Builder**: Interactive plan creation using OpenAI GPT-4 with client context display, conversation memory, streaming responses, plan preview, save/publish workflow, and professional PDF generation. Implements optimistic UI updates for instant message display with full race-condition protection during cross-client navigation. **Persistent sidebar rail redesign** (November 2025): Client Context maintains spatial consistency with persistent 48px rail (w-12) when collapsed, showing vertical toggle and quick KPI chips (age badge, rotated goal snippet). Expands to w-80 with smooth 300ms transitions. **Single-document canvas** (November 2025): Replaced cramped section cards with unified continuous document editor using single Textarea for full-height editing. PDF controls (filename input + download button) moved from top header into canvas header. Canvas features expand/collapse toggle for full-screen editing. Plan content now stored as single text field (backward compatible with legacy sections array format). PDF generation pipeline fixed with proper text rendering, font support, and object storage integration.
