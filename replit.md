...
- **Engagement (UI Scaffold)**: A coach-facing page for AI-powered reminders and client engagement monitoring built with modular components in `/components/engagement/`. Features a two-panel responsive layout (side-by-side on desktop, bottom sheet on mobile). Left panel includes: ActivityTimeline (day-grouped events with color-coded category badges for Meals/Training/Sleep, event-type icons), TriggerList (AI-detected issues with severity badges High/Medium/Low Priority, "Generate Reminder" modals with editable AI-suggested messages), and ReminderSettings (channel toggles for SMS/Push/In-App with tooltips, frequency selector with 4 levels, daily limit slider, quiet hours toggle, test notification button). Right panel includes: RecommendationCard (priority-colored cards with Send/Edit/Dismiss actions and modal interactions), QuickActions (check-in templates triggering message modals), AutoSuggestions (trend/engagement/pattern cards), and NotificationPreview (visual simulations of SMS, push notifications, and in-app alerts). EditMessageModal provides a reusable dialog for editing/sending messages. All interactions trigger toast confirmations. Currently uses mock data; backend integration pending.
