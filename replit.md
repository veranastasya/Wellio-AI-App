...
- **Communication**: Complete two-way messaging system enabling coaches and clients to exchange messages. Features intelligent client list sorting (unread messages first, then by recent activity, then alphabetically), unread message indicators with auto-mark-as-read when conversations are opened, separate coach endpoints (`/api/coach/messages`) for coach-specific functionality, and real-time conversation threading. Clients access messaging via their authenticated portal at `/client/chat`.
