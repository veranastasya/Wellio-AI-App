...
- **AI-Assisted Plan Builder**: Interactive plan creation using OpenAI GPT-4 with client context display, conversation memory, streaming responses, plan preview, save/publish workflow, and professional PDF generation. Implements optimistic UI updates for instant message display with full race-condition protection during cross-client navigation. **Client Context panel** automatically includes pinned questionnaire responses, displaying questionnaire name and submission date for AI-powered personalization. **Horizontal sliding sidebar** (November 2025): Client Context refactored as a collapsible horizontal sidebar with smooth 300ms slide animation on desktop (w-80 ↔ w-0). On mobile (< lg breakpoint), sidebar stacks vertically at full width with max-h-96 when open, or completely hides (w-0 h-0) when closed. AI Chat and Canvas panels expand to fill full width when sidebar is collapsed. Includes full accessibility support (aria-hidden, tabIndex, pointer-events-none) and toggle buttons (PanelLeftClose in sidebar header, PanelLeftOpen in AI Chat header when closed).
