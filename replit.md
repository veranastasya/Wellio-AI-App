...
- **AI-Assisted Plan Builder**: Interactive plan creation using OpenAI GPT-4 with AI Chat and Canvas sections for streamlined workflow, conversation memory, streaming responses, and professional PDF generation. Implements optimistic UI updates for instant message display with full race-condition protection during cross-client navigation. **UI Simplification** (November 2025): Removed Client Context sidebar per user request, leaving clean two-section layout (AI Chat + Canvas) that adapts responsively between desktop (side-by-side) and mobile (stacked) views. **Single-document canvas** (November 2025): Unified continuous document editor using single Textarea for full-height editing. PDF controls (filename input + download button) integrated into canvas header. Canvas features expand/collapse toggle for full-screen editing. Plan content stored as single text field in `{ content: string }` format (backward compatible with legacy sections array format). **PDF generation and access control** (November 2025): Complete ACL system implementation with CLIENT_ID access group type for client-specific files. PDFs stored in object storage with proper owner-based access (coaches as owners) and ACL rules for client READ permission. Fixed 401 Unauthorized errors by: (1) adding CLIENT_ID access group type to handle client access patterns, (2) updating /objects/* route handler to recognize both coach and client sessions, (3) correcting ACL policy generation to use proper enum values. **PDF content and formatting enhancements** (November 2025): Resolved "No content available" issue by updating PDF generation to recognize `{ content: string }` format saved by frontend. Enhanced PDF formatting with markdown heading detection (#, ##, ###), bullet points (-, *, •), numbered lists, section headers (lines ending with :), ALL CAPS headers, **inline bold (`**text**`) and italic (`*text*`) formatting** using pdfkit font switching (Helvetica-Bold, Helvetica-Oblique), professional spacing, and brand colors (#28A0AE for headings, #333333 for body text). **Client Profile Integration** (November 2025): Integrated Plan Builder as "Plan" tab in client profile page at `/coach/clients/:clientId`. Removed standalone `/coach/plan-builder/:clientId` route. Implemented `usePlanBuilder` custom hook for state management and `PlanBuilderContent` presentational component for UI. **Tab State Persistence**: Chat messages and canvas content persist across tab switches (Overview ↔ Intake ↔ Plan) by mounting `usePlanBuilder` at component level, resetting state only when navigating to different client. **Plan Assignment System** (November 2025): Complete workflow for assigning wellness plans to clients with automatic PDF generation, email notifications (sendPlanAssignmentEmail with branded HTML template), plan versioning (single active plan per client with automatic archiving of previous active plans), and proper access controls. Assignment endpoint POST /api/client-plans/:id/assign orchestrates full workflow: saves plan, generates PDF, uploads to object storage with ACL permissions, archives old active plans, sets new plan as shared=true and status='active', sends email notification with optional coach message. Client portal displays assigned active plans at /client/plan with proper content rendering and PDF download. Critical authentication fix: Database seed updated to include bcrypt-hashed passwords for all test clients (password: "password" for Sarah Wilson, Alex Thompson, Emma Davis, Lisa Chen).
