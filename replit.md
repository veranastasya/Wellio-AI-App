...
- **Questionnaire Builder**: CRUD for questionnaires with a draft/published workflow, supporting 10 question types with type-specific settings and validation. Includes toggles for 6 standard health metrics. **Units selector** (U.S. Units/Metric) positioned in top-right corner as filter-style component in Standard Health Metrics section header. Default fields list shows "Client Name" as single field. **Client onboarding form** automatically prefills name and email from invite token data while preserving user edits. **Intake Tab System**: Displays all questionnaire submissions in timeline view with metadata (questionnaire name, submission date), expandable Q&A accordions, PDF download capability, and pin/unpin toggle for AI context integration. Includes empty state handling and smart answer formatting.
